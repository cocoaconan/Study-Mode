...
var htmlRenderInto = function (t, e) {
	...
	tplHtmlTable = function (t) {
		...
		for (i.push("<thead>"),
			i.push("<tr>"),
			i.push('<td colspan="4">'),
			t.range === RANGE_TODAY ? i.push("Today data ") :
			t.range === RANGE_AVERAGE ? i.push("Daily averages since " + t.dateStart + " (" + t.daysSinceStart + " day" + (t.daysSinceStart > 1 ? "s" : "") + ")") : 
			t.range === RANGE_ALLTIME && i.push("All-time data since " + t.dateStart + " (" + t.daysSinceStart + " day" + (t.daysSinceStart > 1 ? "s" : "") + ")"),
			i.push("</td>"),
			i.push("</tr>"),
			i.push("</thead>"),
			i.push("<tbody>"),
			e = 0; e < t.domains.length; e++) r = t.domains[e],
	...
	tplElementDoughnut = function (t, e) {
		...
		for (S.setAttribute("xmlns:wt", "http://www.example.com/webtime-tracker"),
			...
			u = 0; u < t.domains.length; u++) A = !1, r = t.domains[u],
			r.graphed ? (tplElementDoughnutWedgeFragment(r, {
					percentageStart: h,
					noData: A,
					graphGap: e,
					graphSize: l,
